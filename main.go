...
func main() {
	...
	// Setup Gin router
	router := setupRouter(authHandler, shareHandler, healthHandler)
	...
	go func() {
		appLogger.LogInfo(context.Background(), "Starting server", "addr", server.Addr, "base_url", cfg.Server.BaseURL)
	...
	appLogger.LogInfo(context.Background(), "Server exited")
}

// setupRouter configures the Gin router with all routes
func setupRouter(authHandler *handlers.AuthHandler, shareHandler *handlers.ShareHandler, healthHandler *handlers.HealthHandler) *gin.Engine {
	...
	api := router.Group("/api")
	{
		...
		api.POST("/:platform/posts", shareHandler.CreatePost)                 // POST /api/x/posts
		api.GET("/:platform/posts/:post_id/stats", shareHandler.GetPostStats) // GET /api/x/posts/1234567890/stats
		api.GET("/:platform/users/:user_id/stats", shareHandler.GetUserStats) // GET /api/x/users/user123/stats
