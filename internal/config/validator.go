...
func (v *ConfigValidator) ValidateOAuth() error {
	providers := map[string]ProviderConfig{
		"youtube":   v.config.OAuth.YouTube,
		"x":         v.config.OAuth.X,
		"facebook":  v.config.OAuth.Facebook,
		"tiktok":    v.config.OAuth.TikTok,
		"instagram": v.config.OAuth.Instagram,
	}
...
func (v *ConfigValidator) GetValidationWarnings() []string {
	...
	// Check for empty OAuth configurations
	providers := map[string]ProviderConfig{
		"youtube":   v.config.OAuth.YouTube,
		"x":         v.config.OAuth.X,
		"facebook":  v.config.OAuth.Facebook,
	...
	for name, provider := range providers {
		if provider.ClientID == "" || provider.ClientSecret == "" {
			warnings = append(warnings, fmt.Sprintf("OAuth provider %s is not configured", name))
		}
	}
